...
def download_sar(region):
    ...
    dates = [filename[-14:-4] for filename in glob.glob('C:/Users/ioia4268/data/ndwi_masks/All annotations/Hjalstaviken*.tif')]
    # image_collection = get_image_collection(product, region)
    for date in dates:
        ...
        folder = 'bulk_export_final'  # Change this to your file destination folder in Google drive
        start_date = date
        # start_date = os.getenv("START_DATE")
        # start_date = sar_image.date().format('yyyy-MM-dd').getInfo()
        aggregate_function = os.getenv("AGGREGATE_FUNCTION")
        file_name = f'{region_name}_{aggregate_function}_{start_date}_sar_{polarization}'
