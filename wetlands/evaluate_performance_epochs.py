...
def rename_prediction_images(test_name, epoch_num, dataset_name):
...
def iterate(test_name, epoch_num, dataset_name):
    ...
    model_performance_dir = f'{performance_dir}/epoch_{epoch_num}_performance/'
    results_dir = os.getenv('RESULTS_DIR') + f'/{dataset_name}/{test_name}'
    aucs_dataframe = pandas.read_csv(f'{results_dir}/epoch_{epoch_num}_areas_under_curve.csv')
    annotations_dir = os.getenv('ANNOTATED_DATA_DIR') + '/' + dataset_name + '/'
    ...
    for annotated_file in annotated_files:
        ...
        prediction_file = model_performance_dir + annotated_file.replace('annotated_vh', 'mosaic').replace('.png', f'_sar_VH_pred_bw.png')
    ...
    for area_name in ious.keys():
        ...
        print(f'Mean {test_name} epoch {epoch_num} IOU', np.asarray(ious[area_name]).mean())
        print(f'Mean {test_name} epoch {epoch_num} Accuracy', np.asarray(accuracies[area_name]).mean())
        ...
        metrics_file = f'{performance_dir}/epoch_{epoch_num}_performance.csv'
...
def visualize_predicted_image(image, model, device, file_name, test_name, epoch_num, dataset_name):
    patch_size = int(os.getenv('PATCH_SIZE'))
    results_dir = os.getenv('RESULTS_DIR') + f'/{dataset_name}/{test_name}'
    ...
    images_dir = f'{results_dir}/epoch_{epoch_num}_exported_images/'
...
def get_prediction_image(tiff_file, band, model, device, test_name, epoch_num, dataset_name):
    ...
    results = visualize_predicted_image(image, model, device, file_name, test_name, epoch_num, dataset_name)
    return results
...
def plot_results(test_name, epoch_num, dataset_name):

    charts_dir = os.getenv('CHARTS_DIR') + f'/{dataset_name}/{test_name}'
    results_dir = os.getenv('RESULTS_DIR') + f'/{dataset_name}/{test_name}'
    # results_file = '/tmp/water_estimates_flacksjon_2018-07.csv'
    results_file = f'{results_dir}/epoch_{epoch_num}_water_estimates.csv'
    data_frame = pandas.read_csv(results_file, usecols=['1.0', 'Date'], index_col=["Date"],  parse_dates=["Date"])

    data_frame.plot(title=test_name)

    plt.savefig(f'{charts_dir}/epoch_{epoch_num}_water_estimates.png')
    # plt.show()


def update_water_estimates(test_name, epoch_num, dataset_name):
    charts_dir = os.getenv('CHARTS_DIR') + f'/{dataset_name}/{test_name}'
    results_dir = os.getenv('RESULTS_DIR') + f'/{dataset_name}/{test_name}'
    ...
    for index, row in data_frame.iterrows():
        ...
        if area_name not in ious:
            ious[area_name] = [0.]
            prev_prediction = rio.open(f'{results_dir}/epoch_{epoch_num}_exported_images/'+ file_name).read()
            days[area_name] = [0.]
            prev_date = date
            ...
            prediction = rio.open(f'{results_dir}/epoch_{epoch_num}_exported_images/'+ file_name).read()
            if days_passed < 60:
    ...
    data_frame.plot(x='Date', y='1.0', kind='scatter', title=f'{test_name} epoch {epoch_num} [{dataset_name}]', c='color')
    plt.savefig(f'{charts_dir}/epoch_{epoch_num}_scatter_new_water_estimates_filtered.png')
    plt.clf()
    areas_under_curve = {}
    for area_name in ious.keys():
        days_skip_winter = [0.]
        ...
        plt.title(f'{test_name} ep. {epoch_num}_{area_name} [{dataset_name}] auc: {str(round(area_under_curve,2))}')
        plt.savefig(f'{charts_dir}/epoch_{epoch_num}_ious_{area_name}_new_water_estimates_filtered.png')
        plt.clf()
    # plt.show()
    auc_dataframe = pandas.DataFrame.from_dict(areas_under_curve)
    auc_dataframe.to_csv(f'{results_dir}/epoch_{epoch_num}_areas_under_curve.csv')
    data_frame.to_csv(f'{results_dir}/epoch_{epoch_num}_new_water_estimates_filtered.csv')


def full_cycle(test_name, epoch_num, dataset_name):
    load_dotenv()
    ...
    if test_name not in ['otsu', 'otsu_gaussian', 'thresholding_2018', 'thresholding_2020']:
        model_file_2018 = os.getenv('MODELS_DIR') + '/' + test_name + '-2018/epoch_' + epoch_num + '.pth'
        model_file_2020 = os.getenv('MODELS_DIR') + '/' + test_name + '-2020/epoch_' + epoch_num + '.pth'
    ...
    for tiff_file in tqdm.tqdm(sorted(filenames)):
        ...
        if year in [2018, 2019]:
            results = get_prediction_image(tiff_dir + '/' + tiff_file, sar_polarization, model_2018, device, test_name, epoch_num, dataset_name)
        elif year in [2020, 2021, 2022]:
            results = get_prediction_image(tiff_dir + '/' + tiff_file, sar_polarization, model_2020, device, test_name, epoch_num, dataset_name)
    ...
    data_frame.to_csv(f'{results_dir}/epoch_{epoch_num}_water_estimates.csv')
...
def main():
    ...
    for epoch in range(1, 31):
        full_cycle(test_name, str(epoch), dataset_name)
        plot_results(test_name, str(epoch), dataset_name)
        update_water_estimates(test_name, str(epoch), dataset_name)
        rename_prediction_images(test_name, str(epoch), dataset_name)
        copy_annotated_images(test_name, str(epoch), dataset_name)
        iterate(test_name, str(epoch), dataset_name)
